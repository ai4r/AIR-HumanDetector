...
def draw_panel_dual(img_old, img_new, profile_list):
    """
    ...
    img_old = cv2.resize(img_old, (640,480))
    ...
    cv2.putText(panel_img, "Old", (w1+40,15), cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)

    panel_img[20:20+h1, 20:20 + w1, :] = img_new

    h2, w2, c2 = img_old.shape

    panel_img[20:20+h2, 20+w1+20:20+w1+20+w2, :] = img_old
...
def demo_total(video_path):
    ...
    tracker = TrackMod_dual(conf_file="cfg/yolo-f.cfg", model_file=8000, det_threshold=DET_THRESHOLD,
    ...
    while True:
        ...
        img_resize_draw_old = img_resize.copy()
        img_resize_draw_new = img_resize.copy()

        tracker.run(img_resize)

        tracker.draw_old(img_resize_draw_old)
        tracker.draw_new(img_resize_draw_new)
        ...
        panel = draw_panel_dual(img_resize_draw_old, img_resize_draw_new, tracker.tracking_new.profile_classifier.profile_list)

        cv2.imshow("Tracking Result", panel)

        print(frame_idx)
        print('New')
        for trackRes in tracker.current_tracks_new:
            id = trackRes.id
            x1 = trackRes.tl[0]
            y1 = trackRes.tl[1]
            x2 = trackRes.br[0]
        ...
        print('Old')
        for trackRes in tracker.current_tracks_old:
            id = trackRes.id
            x1 = trackRes.tl[0]
            y1 = trackRes.tl[1]
            x2 = trackRes.br[0]
            ...
            print(str(type) + ',' + str(id) + ',' + last_state + ',' + str(x1) + ',' + str(x2) + ',' + str(
                y1) + ',' + str(y2))
