...
def build_train_op(self):
    ...
    if self.optimizer_var is not None:
        self.train_op2 = optimizer.minimize(self.framework.loss, var_list=self.optimizer_var)

    # Training loss for parts
    for info in self.partInfoList:
        grad = self.optimizer.compute_gradients(info.loss)
        info.train_op = self.optimizer.apply_gradients(grad)
