...
class parallel_merge(BaseOp):
	def forward(self):
		self.merge_count = len(self.inp) - 1
		self.out = tf.concat(self.inp[1:], 3)
		self.inp = self.inp[0]

	def speak(self):
		msg = 'parallel merge {}'
		return msg.format(self.merge_count)

class merge(BaseOp):
	def forward(self):
		last_layer = None
		output_list = []
		for name in self.lay.option.input_layer:
			output_list.append(self.named_layers[name].out)
			last_layer = self.named_layers[name]

		self.merge_count = len(output_list)
		self.out = tf.concat(output_list, 3)
		self.inp = last_layer

	def speak(self):
		msg = 'merge {}'
		return msg.format(len(self.lay.option.input_layer))
