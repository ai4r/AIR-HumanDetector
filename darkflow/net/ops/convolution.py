...
class convolutional(BaseOp):
    def forward(self):
        ...
        # Implicit Merge by 'input_layer' attribute
        if self.lay.option.input_layer is not None:
            output_list = []
            for name in self.lay.option.input_layer :
                prev_out = tf.pad(self.named_layers[name].out, [[0, 0]] + pad + [[0, 0]])
                output_list.append(prev_out)

            if len(output_list) == 1:
                temp = output_list[0]
            else:
                temp = tf.concat(output_list, axis=3)

        # Normal Vertical Structure
        else:
            if hasattr(self.inp, 'out'):
                temp = tf.pad(self.inp.out, [[0, 0]] + pad + [[0, 0]])
            else:
                temp = tf.pad(self.inp.out, [[0, 0]] + pad + [[0, 0]])
