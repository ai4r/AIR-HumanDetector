...
def cfg_yielder(model, binary):
	...
	class OptionalParam:
		def __init__(self, other=None):
			...
			if other is not None:
				...
				self.skip_load = other.skip_load
	...
	for i, d in enumerate(layers):
		...
		option.name = d.get('name')
		option.input_layer = d.get('input_layer')
		option.skip_load = (d.get('skip_load') == 1)

		# Implicit merge by input_layers name, re-calculate input size
		if option.input_layer is not None:
			option.input_layer = option.input_layer.split(',')

			w = 0
			h = 0
			c = 0
			for name in option.input_layer :
				try :
					size = layers_dict[name]
					h1, w1, c1 = size

					if w == 0 :
						w = w1
					elif w != w1:
						raise Exception('Layer sizes are inconsistent! %s', option.input_layer)
					...
					raise Exception('input_layer [%s] is not found!' % name)
		...
		#-----------------------------------------------------
			...
			yield ['local', i, size, c, n, stride, 
					pad, w_, h_, activation, option]
			if activation != 'linear': yield [activation, i, option]
			w, h, c = w_, h_, n
			l = w * h * c
		#-----------------------------------------------------
			...
			is_parallel = d.get('is_parallel', 0)
			if pad: padding = size // 2
			activation = d.get('activation', 'logistic')
			batch_norm = d.get('batch_normalize', 0) or conv

			conv_option = OptionalParam(option)

			# If convolutional layer has child activation layer, change name
			if activation != 'linear' and conv_option.name is not None:
				conv_option.name += '_C'

			if parallel_count > 0 and not is_parallel:
				yield ['convolutional', i, size, parallel_count, n, stride, padding, batch_norm,
					   activation, is_parallel, conv_option]
			else:
				yield ['convolutional', i, size, c, n, stride, padding, batch_norm,
				   		activation, is_parallel, conv_option]

			if activation != 'linear':
				yield [activation, i, option]
		...
		#-----------------------------------------------------
			...
			yield ['maxpool', i, size, stride, padding, option]
		...
		#-----------------------------------------------------
			...
			flat = True; l = c
			yield ['avgpool', i, option]
		#-----------------------------------------------------
		elif d['type'] == '[softmax]':
			yield ['softmax', i, d['groups'], option]
		#-----------------------------------------------------
		elif d['type'] == '[connected]':
			if not flat:
				yield ['flatten', i, option]
				flat = True
			activation = d.get('activation', 'logistic')
			yield ['connected', i, l, d['output'], activation, option]
			if activation != 'linear': yield [activation, i]
			l = d['output']
		#-----------------------------------------------------
		elif d['type'] == '[dropout]': 
			yield ['dropout', i, d['probability'], option]
		#-----------------------------------------------------
		elif d['type'] == '[select]':
			if not flat:
				yield ['flatten', i, option]
		...
		#-----------------------------------------------------
			...
			yield ['conv-extract', i, size, c_, n, 
				   stride, padding, batch_norm,
				   activation, inp_layer, out_layer, option]
			if activation != 'linear': yield [activation, i, option]
			w_ = (w + 2 * padding - size) // stride + 1
			h_ = (h + 2 * padding - size) // stride + 1
		...
		#-----------------------------------------------------
			...
			d['old'] = old
			yield ['extract', i] + old + [activation] + [inp_layer, out_layer, option]
			if activation != 'linear': yield [activation, i, option]
			l = len(out_layer)
		#-----------------------------------------------------
			...
			for j, x in enumerate(routes):
				...
				if j == 0:
					...
					c += c_
			yield ['route', i, routes, option]
			l = w * h * c
		#-----------------------------------------------------
		elif d['type'] == '[reorg]':
			stride = d.get('stride', 1)
			yield ['reorg', i, stride, option]
			w = w // stride; h = h // stride; 
			c = c * (stride ** 2)
			l = w * h * c
		# -----------------------------------------------------
		elif d['type'] == '[parallel-merge]':
			if parallel_count > 0:
				c = parallel_count
				parallel_count = 0
			l = w * h * c
			yield ['parallel-merge', i, option]
		# -----------------------------------------------------
		elif d['type'] == '[merge]':
			yield ['merge', i, option]
		#-----------------------------------------------------
		else:
